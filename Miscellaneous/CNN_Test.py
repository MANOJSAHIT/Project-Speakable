...
while(vs.isOpened()):
    ...
    if ok:
        ...
        cv2.rectangle(cv2image, (int(0.6*frame.shape[1]), 0), (frame.shape[1], int(0.5*frame.shape[0])), (255,0,0) ,1)
        roi = frame[0:int(0.5*frame.shape[0]), int(0.6*frame.shape[1]):frame.shape[1]]
        gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
        cv2.imshow("Video LiveFeed", cv2image)
        if dataset==1:
            predictASL(gray)
        elif dataset==2:
            predictISL(gray)
        clear_output(wait=True)
        print("Character:",current_symbol)
        print("Word: ", word)
